...
- [x] One module (`tools/cli/flags.ts`) exports the `FlagSpec` type and a generic `parseArgs` that takes the
      ...
      receives).
- [x] `tools/code-complexity/index.ts` and `tools/mutation-testing/index.ts` import it; the duplicated
...
- [x] `tools/code-complexity/tsconfig.json` gets `rootDir: ".."` (as `tools/mutation-testing/tsconfig.json`
      already has) and `run.sh` plus the `CLI_ENTRY_PATH` guard in `index.ts` point at the new
      `dist/code-complexity/index.js` location.
- [x] The existing `index.spec.ts` in both tools pass unchanged; the shared module has its own co-located
...
- [x] `pnpm test:code-complexity --help`, `pnpm test:mutation-testing --help`, and a `--changed` run of each
      still work from the repo root.
- [x] `pnpm lint:eslint`, `pnpm lint:tsc`, `pnpm lint:doc-links` clean; both tools' `CONTEXT.md` file maps
...
- [x] 1 `[agent]` Extract the parser into `tools/cli/flags.ts` with `flags.spec.ts`
- [x] 2 `[agent]` Rewire `tools/code-complexity` (tsconfig `rootDir`, `run.sh`, entry guard) and
      `tools/mutation-testing` onto it; run both tools' specs and `--help`
- [x] 3 `[agent]` One-line file-map mentions in both `CONTEXT.md`s; lint + doc-links
