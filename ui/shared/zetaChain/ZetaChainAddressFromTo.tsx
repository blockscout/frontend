...
const ZetaChainAddressFromTo = ({ tx, isLoading }: { tx: ZetaChainCCTX; isLoading?: boolean }) => {
  ...
  const senderChain = tx.source_chain_id.toString() === config.chain.id ? undefined :
    (chainsConfig?.find((chain) => chain.chain_id.toString() === tx.source_chain_id.toString()) || null);
  const receiverChain = tx.target_chain_id.toString() === config.chain.id ? undefined :
    (chainsConfig?.find((chain) => chain.chain_id.toString() === tx.target_chain_id.toString()) || null);
