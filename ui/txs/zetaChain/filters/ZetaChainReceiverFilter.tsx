...
const ReceiverFilterInput = ({ address, onChange, onClear, isLast, onAddFieldClick }: InputProps) => {
  return (
    <Flex alignItems="center" w="100%">
      ...
      >
        <Input value={ address } onChange={ onChange } placeholder="Receiver address" size="sm" autoComplete="off"/>
      </InputGroup>
      { isLast && (
        <AddButton
          ml={ 2 }
...
const ZetaChainReceiverFilter = ({ value = [], handleFilterChange, onClose }: Props) => {
  ...
  const onFilter = React.useCallback(() => {
    ...
    handleFilterChange(FILTER_PARAM_RECEIVER, addresses.length > 0 ? addresses : undefined);
    onClose?.();
