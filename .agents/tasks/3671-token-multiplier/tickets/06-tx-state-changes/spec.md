...
- [x] `getStateElements` renders before / after / change through `AssetValue` (no exchange rate — state
...
- [x] A state change with `ui_multiplier: null` renders unscaled with no tag through the predicate's
      existing `undefined` path (Q01); no fallback to the token's current factor.
- [x] Non-ERC-8056 token state changes render the same numbers as before (existing `TxState.pw.tsx`
      baselines unchanged apart from the tooltip now existing).
- [x] Playwright scaffold: `TxState.pw.tsx` gains an ERC-8056 row from a new state-change mock.
- [x] `(human)` Before/after/change values are `1.69 ×` the raw ones, tagged, with the tooltip.
...
- [x] 1 `[agent]` Move `getStateElements` amounts onto `AssetValue` with the multiplier; state-change mock;
      Playwright scaffold
- [x] 2 `[human]` Check the tag fits the state table's numeric cells; generate baselines
