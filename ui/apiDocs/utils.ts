...
const microserviceRequestInterceptorFactory =
  (api: ApiPropsBase) => (req: SwaggerRequest) => {
    try {
...
const getMicroserviceSwaggerUrl = (api: ApiPropsBase) =>
  `${api.endpoint}${api.basePath ?? ""}/api/v1/docs/swagger.yaml`;
...
export const REST_API_SECTIONS = [
  ...
  config.apis.bens && {
    ...
    swagger: {
      ...
      requestInterceptor: microserviceRequestInterceptorFactory(
        config.apis.bens
      ),
    },
  },
