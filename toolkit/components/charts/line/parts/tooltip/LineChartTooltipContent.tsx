...
const CLASS_NAME = 'LineChartTooltip__content';

const LineChartTooltipContent = ({ children }: Props) => {
  return <g className={ CLASS_NAME }>{ children }</g>;
};

export default React.memo(LineChartTooltipContent);
...
export function useRenderContent(ref: React.RefObject<SVGGElement | null>, { chart, transitionDuration }: UseRenderContentParams) {
  return React.useCallback((x: number, y: number) => {
    const tooltipContent = d3.select(ref.current).select(`.${ CLASS_NAME }`);
