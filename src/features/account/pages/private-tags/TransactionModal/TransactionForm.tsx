...
import type { TransactionTag, TransactionTagErrors } from 'src/features/account/types/api';

import useApiFetch from 'src/api/hooks/useApiFetch';
import type { ResourceErrorAccount } from 'src/api/resources';
import { resourceKey } from 'src/api/resources';

import getErrorMessage from 'src/features/account/utils/get-api-error-text';

import { Button } from 'src/toolkit/chakra/button';
import { FormFieldText } from 'src/toolkit/components/forms/fields/FormFieldText';
import { TRANSACTION_HASH_LENGTH, TRANSACTION_HASH_REGEXP } from 'src/toolkit/components/forms/validators/transaction';
...
const TransactionForm: React.FC<Props> = ({ data, onOpenChange, onSuccess, setAlertVisible }) => {
  ...
  const { mutateAsync } = useMutation({
    mutationFn: (formData: Inputs) => {
      ...
      if (isEdit) {
        return apiFetch('core:private_tags_tx', {
          pathParams: { id: String(data.id) },
          fetchParams: { method: 'PUT', body },
        });
      }

      return apiFetch('core:private_tags_tx', { fetchParams: { method: 'POST', body } });
    },
    ...
    onSuccess: async() => {
      await queryClient.refetchQueries({ queryKey: [ resourceKey('core:private_tags_tx') ] });
      await onSuccess();
