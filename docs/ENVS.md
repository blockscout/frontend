...
| search | `{ background?: [string, string]; border_width?: [string, string] }` | Search bar customization. `background` sets the input background color (valid `background-color` CSS value). `border_width` sets the border width (in px). | - | - | `{ 'background': ['white', '#1A202C'], 'border_width': ['0px', '2px'] }` |
