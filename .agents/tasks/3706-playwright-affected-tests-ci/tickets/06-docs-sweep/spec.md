...
- [ ] `.agents/rules/tests-visual.md` "File naming and location" block lists `pnpm test:pw`, `pnpm test:pw
...
- [ ] `docs/CONTRIBUTING.md`: the Playwright paragraph and the command table drop `test:pw:local` /
      `test:pw:ci`, keep `test:pw:docker[:deps]` as aliases, and mention `--changed`.
- [ ] `.vscode/tasks.json`: every pw task and input option references only commands that exist
      (`test:pw`, `--docker`, `--changed`); the `pwArgs` presets that named `--affected` name `--changed`.
- [ ] `tools/playwright/CONTEXT.md` final pass against `.agents/rules/docs.md`: the constraint that the
...
- [ ] `grep -rn "test:pw:local\|test:pw:ci\|detect-affected\|--affected\|pw\.sh" --exclude-dir=node_modules
      --exclude-dir=.git .` returns only `.agents/tasks/` hits.
- [ ] `pnpm lint:doc-links`, `pnpm lint:cspell` clean.

## Leaf worklist

- [ ] 1 `[agent]` `.agents/rules/tests-visual.md`, `docs/CONTRIBUTING.md`, `.vscode/tasks.json`
- [ ] 2 `[agent]` `tools/playwright/CONTEXT.md` final pass; grep sweep; lint
