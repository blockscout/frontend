...
| NEXT_PUBLIC_MARKETPLACE_CONFIG_URL | `string` *(optional)* | URL of configuration file (`.json` format only) which contains list of apps that will be shown on the marketplace page. See [below](#marketplace-app-configuration-properties) list of available properties for an app | `https://example.com/marketplace_config.json` |
