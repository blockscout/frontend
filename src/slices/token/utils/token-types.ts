...
export function isFungibleTokenType(typeId: TokenType | undefined | null, chainConfig?: ChainConfig): boolean {
  return typeId === 'ERC-20' || typeId === 'NATIVE' || getAdditionalTokenTypes(chainConfig).some((item) => item.id === typeId);
}

export function hasTokenTransferValue(typeId: TokenType | undefined | null, chainConfig?: ChainConfig) {
...
export function isConfidentialTokenType(typeId: TokenType | undefined | null): boolean {
  return typeId === 'ERC-7984';
}

export function hasTokenIds(typeId: TokenType | undefined | null) {
