...
export const base = async <Pathname extends Route['pathname'] = never>({
  ...
  // 添加翻译数据
  const translations = await serverSideTranslations(locale || 'zh', ['common', 'order']);
  ...
  console.log('getServerSideProps base result:', JSON.stringify(props, null, 2));
...
export const deposits: GetServerSideProps<Props> = async (context) => {
  if (
    !(
      ...
      (rollupFeature.type === 'optimistic' || rollupFeature.type === 'shibarium' || rollupFeature.type === 'zkEvm')
    )
  ) {
    return {
      notFound: true,
