...
function toTarget(file: string, changedRanges: ReadonlyArray<LineRange> | undefined): MutateTarget | undefined {
  const ranges = computeMutationRanges(fs.readFileSync(file, 'utf8'), file, changedRanges);
...
function toWholeFileTargets(files: ReadonlyArray<string>): Array<MutateTarget> {
  return files.map((file) => toTarget(file, undefined)).filter(isTarget);
}

// Full mode: every in-scope file in the repo that has a spec beside it.
function selectAll(): Selection {
  const eligible = getAllSourceFiles().filter(hasCoLocatedSpec);
  if (eligible.length === 0) return { outcome: 'empty', reason: 'No source file in scope has a co-located vitest spec — nothing to mutate.' };

  const targets = toWholeFileTargets(eligible);
...
function selectFocused(focusPaths: ReadonlyArray<string>): Selection {
  ...
  const eligible = paths.filter(hasCoLocatedSpec);
...
function selectChanged(baseRef: string): Selection {
  ...
  const eligible = changed.filter(hasCoLocatedSpec);
  ...
  const targets = eligible.map((file) => toTarget(file, getChangedLineRanges(baseCommit, file))).filter(isTarget);
