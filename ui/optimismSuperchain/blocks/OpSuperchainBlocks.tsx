...
const OpSuperchainBlocks = () => {
  ...
  const blocksQuery = useQueryWithPages({
    ...
    options: {
      ...
      placeholderData: generateListStub<"general:blocks">(BLOCK, 50, {
        next_page_params: {
  ...
  const tabs: Array<TabItemRegular> = [
    {
      ...
      title: "All",
      component: (
        <OpSuperchainBlocksContent
          type="block"
          query={blocksQuery}
          chainSlug={blocksQuery.chainValue?.[0]}
        />
      ),
    },
    {
      ...
      component: (
        <OpSuperchainBlocksContent
          type="reorg"
          query={reorgsQuery}
          chainSlug={reorgsQuery.chainValue?.[0]}
        />
      ),
    },
    {
      ...
      component: (
        <OpSuperchainBlocksContent
          type="uncle"
          query={unclesQuery}
          chainSlug={unclesQuery.chainValue?.[0]}
        />
      ),
    },
  ];
  ...
  const currentChainInfo = multichainConfig()?.chains.find(
    (chain) => chain.slug === currentQuery.chainValue?.[0]
  );
  ...
  const rightSlot = (
    <HStack gap={8} hideBelow="lg">
      <Link
        href={route(
          { pathname: "/block/countdown" },
          currentChainInfo ? { chain: currentChainInfo } : undefined
        )}
      >
