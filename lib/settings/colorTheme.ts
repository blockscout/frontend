...
export function getThemeHexWithOverrides(colorThemeId: ColorThemeId, overrides?: Record<string, unknown>) {
  ...
  if (colorThemeId === 'light') {
    const value = getNestedValue(appliedOverrides, 'bg.primary._light.value');
    return typeof value === 'string' ? value : defaultHex;
  }

  if (colorThemeId === 'dark') {
    const value = getNestedValue(appliedOverrides, 'bg.primary._dark.value');
    return typeof value === 'string' ? value : defaultHex;
  }
