...
const TokenMultiplierHistoryInline = ({ hash, changesCount }: Props) => {
  ...
  const content = (() => {
    if (query.isError) {
      return <Text textStyle="sm" color="text.secondary" mt={ 2 } w="100%">Unable to load the multiplier history.</Text>;
    }

    if (!items || items.length === 0) {
      return <Text textStyle="sm" color="text.secondary" mt={ 2 } w="100%">No multiplier changes found.</Text>;
    }
