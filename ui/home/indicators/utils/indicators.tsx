...
const INDICATORS: Array<TChainIndicator> = [
  {
    ...
    valueMicroservice: (stats) =>
      stats.yesterday_transactions?.value === null
        ? "N/A"
        : Number(stats.yesterday_transactions?.value).toLocaleString(
            undefined,
    ...
    hintMicroservice: (stats) =>
      stats.daily_new_transactions?.info?.description,
  },
  {
    ...
    valueMicroservice: (stats) => {
      if (isArbitrumRollup) {
        return stats.yesterday_operational_transactions?.value === null
          ? "N/A"
          : Number(
              stats.yesterday_operational_transactions?.value
        ...
        return stats.op_stack_yesterday_operational_transactions?.value === null
          ? "N/A"
          : Number(
              stats.op_stack_yesterday_operational_transactions?.value
            ).toLocaleString(undefined, {
              maximumFractionDigits: 2,
              notation: "compact",
            });
    ...
    hintMicroservice: (stats) =>
      stats.daily_new_operational_transactions?.info?.description,
  },
  {
    id: "coin_price",
    ...
    valueDiff: (stats) =>
      stats?.coin_price !== null ? stats?.coin_price_change_percentage : null,
  ...
  },
  {
    ...
    value: (stats) =>
      stats.tvl === null
        ? "$N/A"
        : "$" +
          Number(stats.tvl).toLocaleString(undefined, {
            maximumFractionDigits: 2,
            notation: "compact",
          }),
    icon: (
