...
- [ ] `tac:operations` resolves to `/api/v2/tac/operations`, with `q`, `page_token` and `page_items`
      unchanged, and its payload typed from the v2 package types
- [ ] The status presentation is derived from `status` only; no code path in the list reads `type` to decide
      an outcome
- [ ] The route label is derived from `type` and renders for `pending` operations
- [ ] `rollback: true` renders as a tag beside the status tag, for both rollback and non-rollback failures
- [ ] `error_reason` appears in the status tooltip when present; the tooltip falls back to the plain failure
      text when absent
- [ ] `type: UNKNOWN` renders the pending presentation with no route and no layout breakage
- [ ] A Vitest spec covers the label and tooltip branching across every `status` × `rollback` ×
      `error_reason` × `UNKNOWN` combination
- [ ] `(human)` The list matches the mockup — tag colours, icons, spacing, and the rollback tag's placement
      and tooltip copy
- [ ] `(human)` Search (`q`), pagination and sender rendering behave exactly as before on the same data
...
- [ ] 1 `[agent]` Repoint the three tac resources to `/api/v2/tac/...` and retype their payloads — skill: `add-api-resource`
  - inputs:
...
- [ ] 2 `[agent]` Rebuild `TacOperationStatus` around `status` + `type` + `error_reason`, and extract the rollback tag as its own component
- [ ] 3 `[agent]` Wire the table and mobile-list rows to the new props, including the pending spinner and the `UNKNOWN` case
- [ ] 4 `[agent]` Vitest spec for the route-label and tooltip-text branching
- [ ] 5 `[agent]` Extend the `TacOperationStatus.pw.tsx` cases to one per visual variant and update the tac mocks and stubs to the v2 shape
- [ ] 6 `[human]` Style the status tag and the rollback tag to the mockup, then regenerate the screenshot baselines — [Figma](https://www.figma.com/design/1UWWsK0bg6ifzS9O1NLlo4/TAC-TON-TAC-operations?node-id=4001-37444)
