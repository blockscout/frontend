...
10. The interface has two independent axes, mirroring vitest. **Selection** — which functions to
    score — is *full repo* by default (every in-scope `src/**` function), *focused* when explicit
...
11. **Full-repo mode** (the default selection) scores every in-scope function and is what ticket 04
    uses to produce the repo-wide CRAP distribution before thresholds are set. **Focused mode**
