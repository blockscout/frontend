...
dotenv \
  ...
  -e $config_file \
  -e $secrets_file \
  -- bash -c './deploy/scripts/make_envs_script.sh && if [ -z "$NEXT_PUBLIC_APP_LISTEN_PORT" ]; then export NEXT_PUBLIC_APP_LISTEN_PORT=$NEXT_PUBLIC_APP_PORT fi && next dev -- -p $NEXT_PUBLIC_APP_LISTEN_PORT' |
pino-pretty