...
- [x] `pw_changes` job: `if: github.event_name == 'pull_request'`, `actions/checkout@v4` with
...
- [x] `pw_tests`: `needs: [code_quality, envs_validation, pw_changes]`; `if:` is `always()` with the two
...
- [x] `strategy.matrix.include` has five entries with `name` and `project` (+ `shard` for `default-1..3`,
      `1/3`…`3/3`); the job title uses `matrix.name`; the blob artifact is `blob-report-${{ matrix.name }}`
      so shards do not collide.
- [x] Run step: `pnpm test:pw ${{ github.event_name == 'pull_request' && '--changed' || '' }}
...
- [x] Matrix checkout is `actions/checkout@v4` with `fetch-depth: 0` and no `lfs`; the "Install git-lfs"
...
- [x] `pw_report` unchanged except it still merges whatever `blob-report-*` artifacts exist (the
      `has_reports` guard already handles zero).
- [x] The comment in `playwright-ct.config.ts` above `projects` ("when adding or deleting a project, update
      the github workflow") still holds; extend it to mention the shard count.
- [x] `deploy/tools/affected-tests/` deleted; its entry removed from `pnpm-workspace.yaml`, the
...
- [x] A `workflow_dispatch` run of `Checks` on the branch (`gh workflow run checks.yml --ref issue-3706`,
...
- [x] 1 `[agent]` `checks.yml`: `pw_changes` gate, matrix `include`, run command, lfs/cache removal,
      artifact names
- [x] 2 `[agent]` Delete `deploy/tools/affected-tests` and its references; reinstall; update the
      `projects` comment in `playwright-ct.config.ts`
- [x] 3 `[agent]` Dispatch the workflow on the branch, watch it, record the run in `notes.md`; lint
