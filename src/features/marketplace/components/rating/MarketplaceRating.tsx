...
import { Rating as RatingComponent } from 'src/toolkit/chakra/rating';
import { Skeleton } from 'src/toolkit/chakra/skeleton';

import Content from './MarketplaceRatingPopoverContent';
import TriggerButton from './MarketplaceRatingTriggerButton';
    ...
    >
      { fullView && (
        <>
          ...
          { Boolean(rating) && <Text fontSize="md" ml={ 2 }>{ rating }</Text> }
          { Boolean(ratingsTotalCount) && <Text color="text.secondary" textStyle="md" ml={ 1 }>({ ratingsTotalCount })</Text> }
        </>
      ) }
