...
import type { AddressTag, TransactionTag, AddressTagsResponse, TransactionTagsResponse } from 'src/features/account/types/api';
...
const DeletePrivateTagModal: React.FC<Props> = ({ open, onOpenChange, data, type }) => {
  ...
  const mutationFn = useCallback(() => {
    const resourceName = type === 'address' ? 'core:private_tags_address' : 'core:private_tags_tx';
  ...
  const onSuccess = useCallback(async() => {
    if (type === 'address') {
      queryClient.setQueryData(getResourceKey('core:private_tags_address'), (prevData: AddressTagsResponse | undefined) => {
        const newItems = prevData?.items.filter((item: AddressTag) => item.id !== id);
        return { ...prevData, items: newItems };

      });
    } else {
      queryClient.setQueryData(getResourceKey('core:private_tags_tx'), (prevData: TransactionTagsResponse | undefined) => {
