...
const BUTTON_TEXT = 'Buy on Example Portal';
const CONFIGURED_URL = 'https://portal.example.com/swap';
const TOKEN_HASH = '0x7Fc66500c84A76Ad7e9c93437bFc5Ac33E2DDaE9';
const TOKEN_HASH_LOWERCASE = TOKEN_HASH.toLowerCase();

const envs: Array<[ string, string ]> = [
  [ 'NEXT_PUBLIC_TOKEN_ACTION_BUTTON_CONFIG', `{"text":"${ BUTTON_TEXT }","url":"${ CONFIGURED_URL }"}` ],
];

const envsWithPlaceholder: Array<[ string, string ]> = [
  [ 'NEXT_PUBLIC_TOKEN_ACTION_BUTTON_CONFIG', `{"text":"${ BUTTON_TEXT }","url":"${ CONFIGURED_URL }/{hash}?chainId=1&token={hash}"}` ],
];
...
it('renders nothing when the button is not configured', async() => {
  ...
  render(<TokenActionButton tokenHash={ TOKEN_HASH } tokenType="ERC-20"/>);
...
it('renders nothing for a token that is not ERC-20', async() => {
  await withEnvs(envs, async() => {
    ...
    render(<TokenActionButton tokenHash={ TOKEN_HASH } tokenType="ERC-721"/>);
...
it('renders while the token type is still unknown', async() => {
  await withEnvs(envs, async() => {
    ...
    render(<TokenActionButton tokenHash={ TOKEN_HASH } tokenType={ undefined } isLoading/>);
...
it('substitutes every {hash} placeholder with the lowercased token hash', async() => {
  await withEnvs(envsWithPlaceholder, async() => {
    const { 'default': TokenActionButton } = await import('./TokenActionButton');
    ...
    expect(link.getAttribute('href'))
      .toBe(`${ CONFIGURED_URL }/${ TOKEN_HASH_LOWERCASE }?chainId=1&token=${ TOKEN_HASH_LOWERCASE }&utm_source=blockscout&utm_medium=token`);
  });
});
