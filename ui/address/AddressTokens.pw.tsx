...
const test = base.extend({
  page: async({ page }, use) => {
    ...
    await page.route(API_URL_TOKENS + '?type=DRC-721', (route) => route.fulfill({
      status: 200,
      body: JSON.stringify(response721),
    }));
    await page.route(API_URL_TOKENS + '?type=DRC-1155', (route) => route.fulfill({
      status: 200,
      body: JSON.stringify(response1155),
    }));
    await page.route(API_URL_TOKENS + '?type=DRC-404', (route) => route.fulfill({
      status: 200,
...
base.describe('update balances via socket', () => {
  ...
  test('', async({ mount, page, createSocket }) => {
    ...
    await page.waitForResponse(API_URL_TOKENS + '?type=DRC-721');
    await page.waitForResponse(API_URL_TOKENS + '?type=DRC-1155');
    await page.waitForResponse(API_URL_TOKENS + '?type=DRC-404');
